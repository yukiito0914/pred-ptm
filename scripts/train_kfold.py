...
# [UPDATED] Import matthews_corrcoef
...
def get_args():
    parser = argparse.ArgumentParser(description='Train MLP with K-Fold CV (All Metrics)')
    ...
    parser.add_argument('--output_dir', type=str, default='experiments_kfold', help='Output dir')
...
def validate(model, loader, device, criterion):
    """
    Returns Loss and raw probabilities/targets.
    Also calculates default metrics (th=0.5) for monitoring log.
    """
    model.eval()
    val_loss = 0.0
    all_probs = []
    ...
    # Default metrics (Threshold 0.5) for simple monitoring
...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(all_labels)), all_labels)):
        ...
        for epoch in range(args.epochs):
            ...
            # Save metrics if this is the best model so far
            if val_loss == early_stopping.val_loss_min:
                # 1. Find Optimal Threshold (Maximize F1)
                best_th, best_f1 = find_optimal_threshold(val_targets, val_probs)
                
                # 2. Calculate ALL metrics at that optimal threshold
