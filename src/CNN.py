...
class PhosphoCNN(nn.Module):
    def __init__(self, input_dim=93, hidden_dim=128, output_dim=1):
        """
        Deep 1D-CNN (3 Layers) for Phosphorylation Prediction.
        """
        super(PhosphoCNN, self).__init__()
        ...
        # --- 2. Deep CNN Layers (3 Layers) ---
        self.cnn = nn.Sequential(
            # Layer 1: Local properties
            # Input: (Batch, 10, 31) -> Output: (Batch, 32, 31)
            ...
            # Layer 2: Motifs
            # Input: (Batch, 32, 31) -> Output: (Batch, 64, 31)
            ...
            # Layer 3: Context
            # Input: (Batch, 64, 31) -> Output: (Batch, 128, 31)
        ...
        # Output calculation
        # Window size is maintained at 31 due to padding=1
        # input_dim comes as 93 (31*3). So window_len is 31.
        self.window_len = input_dim // 3  # = 31
        
        # Flatten size = Length(31) * Last_Channels(128) = 3968
        self.cnn_flat_dim = self.window_len * 128
        
        # --- 3. Classifier ---
        # Combine CNN output + Structural features
        total_dense_input = self.cnn_flat_dim + input_dim
    ...
    def forward(self, x_features, x_seq):
        # A. Sequence -> CNN
        emb = self.embedding(x_seq.long()) # (B, 31, 10)
        emb = emb.permute(0, 2, 1)         # (B, 10, 31)
        
        cnn_out = self.cnn(emb)            # (B, 128, 31)
        cnn_flat = cnn_out.view(cnn_out.size(0), -1) # (B, 3968)
